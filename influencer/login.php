...
if (isset($_POST['login_submit'])) {
    ...
    $res = mysqli_query($conn, "select * from influencer where influ_email='$email'");

    if (mysqli_num_rows($res) > 0) {
        $row = mysqli_fetch_assoc($res);
        if ($password===$row['influ_password']) {
           
            $_SESSION['USER_LOGIN_DONE'] = true;
            $_SESSION['USER_LOGIN_ID'] = $row['influ_id'];
            $_SESSION['USER_LOGIN_FNAME'] = $row['influ_firstname'];
            $_SESSION['USER_LOGIN_LNAME'] = $row['influ_lastname'];
            $_SESSION['USER_LOGIN_EMAIL'] = $row['influ_email'];
